public class Reversi {
  ...
  public static void start(Game g) {
    ...
    while (!isEmpty(g.getPlaceableLocations(opponent, player)) || !isEmpty(g.getPlaceableLocations(player, opponent))) {
      /*
       * if (isEmpty(g.getPlaceableLocations(opponent, player))) { continue; }
       */
      char temp = player;
      player = opponent;
      opponent = temp;
    ...
    // g.showPlaceableLocations(g.getPlaceableLocations(player, opponent), player,
    // opponent);
    System.out.println("shouldnt be any stars on board");
    g.gameResult(g.getPlaceableLocations('W', 'B'), g.getPlaceableLocations('B', 'W'));
    g.displayBoard();
    /*
     * Integer coordinates = input.nextInt(); Point p = new Point(); p.y =
     * coordinates.toString().charAt(0); p.x = coordinates.toString().charAt(1);
     * 
     * g.placeMove(p, player, opponent);
     * g.showPlaceableLocations(g.getPlaceableLocations(player, opponent), player,
     * opponent);
